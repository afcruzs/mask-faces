...
class Yolov2(object):
    ...
    def __init__(self, model_file_path):
        self.model_face = load_model(model_file_path)
    ...
    def detect_faces(self, frame):
        ...
        out2 = self.model_face.predict(img_camera)[0]

        yolo_results = interpret_output_yolov2(out2, img.shape[1], img.shape[0])

        results = []

        for i in range(len(yolo_results)):
            x = int(yolo_results[i][1])
            y = int(yolo_results[i][2])
            w = int(yolo_results[i][3])//2
            h = int(yolo_results[i][4])//2
            confidence = yolo_results[i][5]
            ...
            results.append({'confidence': confidence, 'box': [xmin, ymin, xmax - xmin, ymax - ymin]})
